...
export default function Popover({
  ...
  useEffect(() => {
    ...
    document.getElementById(target)?.addEventListener('click', handleOpentoggle)
    document.body.addEventListener("click", handleClickOutside);

    return () => {
      document.body.removeEventListener("click", handleClickOutside);

      document.getElementById(target)?.removeEventListener('click', handleOpentoggle)
    };
  }, [open, buttonClicked]);
      ...
      <div className="relative">
        <>
          {plain ? (
            ...
            <button
              className={`
                 inline-flex items-center max-h-10 ${target !== false && "hidden"
          ...
          {open && (
            <div
              className={`popover absolute  z-20 mt-3 ${width ? width : "w-screen"
